...
- **Classify** — pick a wafer (or upload your own `.npy` map) to get a model prediction with a Grad-CAM overlay showing what drove it, then confirm or refine it into a more specific in-house subtype.
...
1. Verifying a wafer's label in **Classify** or **Defect Reevaluation** writes it as ground truth.
...
3. Promoting a candidate to production is a separate, explicit action in the **Model** tab. This keeps a human expertise in the loop before any newly trained model is actually used for predictions.
...
- In-house subtypes aren't modeled as separate output classes (too few verified examples per subtype); instead they're found via nearest-neighbor search over the network's penultimate-layer embeddings.
- Evaluation mirrors the article's methodology: row-normalized confusion matrix, per-class precision/recall/F1, and precision-recall curves with AUC swept over every distinct prediction score.

## Setup

```bash
git clone <repo-url>
cd wafer-defect-id-model
python -m venv .venv
